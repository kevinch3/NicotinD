...
## Across the variety switcher (`--strategy`)

The A/B above is all `balanced`. The switcher moves the genre weight (18 →
**24** on `similar`, → **8** on `different`) and, on `different`, adds an
out-of-genre pool pass and reserves 30 % of the window for its best rows. Both
ends were re-run on the tech-house seed, `different` also on folclore.

The worry going in was that `COS_FLOOR` 0.75 zeroes a majority of all pairs
(all-pairs p50 is 0.634), so `different`'s out-of-genre rows — selected
precisely for sharing no genre token — would all collapse to 0 and the quota
would fill by tie-break. **Refuted:** they score 0.27–0.79, because "no shared
token" is a lexical fact and the centroid still places them.

What the runs actually expose is the opposite problem in the shipped axis. Count
the **distinct genre-axis values across the served 15**:

| seed / strategy | lexical | learned (tuned) |
| --- | --- | --- |
| tech house · `balanced` | **1** (all 1.00) | 5 |
| tech house · `similar` | **1** (all 1.00) | 3 |
| tech house · `different` | **3** (1.00 · 0.60 · 0.00) | **9** |
| folclore · `different` | **2** (1.00 · 0.60) | **7** |

The lexical rule has only three values it can *ever* return — exact 1.0, token
containment 0.6, disjoint 0 — so on a tight pool it degenerates to a constant
and on a wide one to a three-step ladder. The heaviest axis in the formula is
either inert or nearly so, in every strategy.

- **`similar`** is where the shipped axis is worst and the fix helps most: a
  250-row same-genre pool means all 15 served tracks score **1.00**, so raising
  the genre weight to 24 changes nothing at all — and Backstreet Boys, Gwen
  Stefani and Alexandra Stan still ride in on a shared `Pop` tag. Tuned, the
  list is Colyn / Carl Cox / Joseph Capriati / Eli & Fur / Innellea / Oscar L /
  Green Velvet, and the three pop acts are gone. Mean embedding cosine is flat
  (0.741 → 0.736) — this is the genre axis doing work the audio axis was
  already doing alone.
- **`different`** keeps its guarantee (10/15 share a seed genre, i.e. the 30 %
  quota fills, identically in both runs — it is a pool-and-quota mechanism, not
  a score-driven one) and gains an *ordering* inside the widened part. Lexically
  the last five slots all tie at 0.60 and fall in arbitrary order; tuned they
  rank Die Antwoord 0.79 → Calvin Harris 0.57 → Kardinal Offishall 0.35 →
  Black Eyed Peas 0.28 → Çantamarta 0.27. Mean embedding cosine 0.591 → 0.623.
  On folclore the same slot goes from five tracks tied at 0.60 (incl. Buena
  Vista Social Club and Nana Mouskouri seated at a full 1.00) to a graded
  0.78 → 0.52 → Santaolalla 0.31 — "different but adjacent" instead of
  "different, unordered".

So the switcher and the learned axis are complementary rather than in tension:
the switcher decides *how wide the pool is*, and the axis is what can finally
rank inside it. The one thing to re-check after the v9 flip (#1121) is
`different`'s weight of 8 — it was set against an axis that returned three
values, and a continuous axis at that weight may now be under-used.
