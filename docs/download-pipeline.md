...
**A single-song delete refreshes its album too (issue #774).** `library_albums.song_count` / `duration` are a scan-time snapshot, and `deleteOne` used to remove only the `library_songs` row — so an album kept reporting its pre-delete count until the next *full* scan. Measured on prod during the 2026-08-27 curation pass: deadmau5 `while(1<2)` listed 24 tracks under a header of 40 after 16 dupes were deleted, Ana Tijoux `Kaos` read 24 against 14, and `audit-library`'s `album_song_count_mismatch` rule went 0 → 1 catching exactly this. The count is not cosmetic — it is the `owned` side of the completeness dimension and feeds album cards, so a stale high count makes a freshly-deduped album look complete and hands a curator a wrong denominator.

`deleteOne` now captures the song's `album_id` before the row goes and calls the shared `pruneOrphanAlbum` (`services/library-aggregates.ts`) on both delete paths — the normal one and the already-gone-from-disk one. That helper recomputes the aggregates from the surviving songs and, when the album lost its last one, drops the `library_albums` / `library_album_artists` rows and prunes any artist it orphans — the same cleanup one level down from `pruneOrphanArtist`. It is the single shared form of a recompute that previously existed three times (the scanner's missing-file prune, the scanner's incremental persist, and the metadata-fix re-point), all now routed through `refreshAlbumAggregate` / `pruneOrphanAlbum`. Same family as #771 (`library_genres.song_count`): an aggregate whose refresh paths did not cover every mutation site.
...
**…and the same guard on the addon path (the duplicate-download regression).** The paragraph above describes the *in-process* engine. After the phase-4 cutover, yt-dlp and spotdl are external **addons**, so a pasted YouTube/Spotify link is matched by `resolveAddonForUrl` and never reaches `AcquireWatcher.submit()` — it took the addon branch of `POST /api/acquire`, which had **no** guard at all. Three things then compounded into "Get stays enabled and the same link downloads N times":

1. **No server-side dedupe.** Every click created a new addon job *and* a new `acquisition_jobs` row.
2. **The job was invisible to the surface that started it.** `GET /api/acquire/jobs` read `acquire_jobs`, which an addon job never gets a row in. The web link-intent card tracks its job by matching that list on `url` (`linkJob()`), so the card saw nothing, kept rendering an armed **Get**, and never showed progress. The only surface that *did* show the download was the Downloads tab (which reads `acquisition_jobs`) — hence "the downloads list gets populated too late", which is what prompted the extra clicks.
3. **No client-side re-entrancy guard.** Even with the two above fixed there is a round-trip during which no job exists yet, so a double-tap fired two submits.

The fix, in the same three places:

1. `acquisition_jobs.source_url` (additive column) records the submitted link for `kind='url'` jobs — `source_ref` can't, it holds the `addon:<id>:<jobId>` back-reference the poller keys on. `POST /api/acquire` then calls `findInFlightAddonUrlJob(db, url)` first and returns the existing job with **200 + `{ reused: true }`** instead of starting a second download (201 still means "created"). It also passes an `Idempotency-Key` of `url:<href>` to the addon so the addon's own guard engages, and treats an addon **409** as "already in flight" rather than an error the user would retry.
...
**Method badge mapping covers the slskd addon (issue #532)**: `methodForBackend` (`lib/download-groups.ts`) normalizes addon ids to their base method (`ytdlp-addon`→`ytdlp`, `bundled-archive`→`archive`, and — the cutover regression — `slskd`/`slskd-addon`→`slskd`); anything unmapped renders the `unknown` "?Unknown source" badge, which is exactly how every album-hunt card looked post-cutover while `BADGES.slskd` existed unused.

**Every feed row is removable (issue #533)**: `DELETE /api/downloads/jobs/:id` deletes the core `acquisition_jobs` row unconditionally, proxying cancel/delete to the owning addon **only** when `source_ref` parses as `addon:<id>:<jobId>` and best-effort at that. It used to 400 ("use the transfer routes" — routes that no longer exist since phase 3) for every pre-cutover row (peer-username refs), `kind:'direct'` grab and url mirror row, and the web `.catch(() => {})`-ed the 400 — Remove and "Clear finished" silently no-opped, the user-visible "old downloads I can't remove". The web now surfaces a failed removal as an error toast (one aggregate toast for Clear finished); `POST /jobs/:id/cancel` keeps its 400 for non-addon rows (nothing is in flight to cancel) but logs it. The 7-day `FINISHED_JOB_TTL_MS` boot prune remains the history bound.

**Remove is audited (issue #1086)**: both `DELETE /api/downloads/jobs/:id` and its twin `DELETE /api/acquire/jobs/:id` (addon-run URL jobs) write a `download.remove` audit row (`target_kind: 'acquisition_job'`, the job id), whose `detail` says what happened addon-side: `released addon job <addon>:<job>`, `addon delete of … failed (best-effort): <error>`, the addon being unavailable, or `no addon job` for legacy/url-mirror rows. Before this a deleted card could only be inferred by elimination once container logs rotated (#1081). In-process watcher jobs on the acquire route are not `acquisition_jobs` rows and are not audited.
