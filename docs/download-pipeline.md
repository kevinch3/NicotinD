...
**Resume after truncation (spotdl)**: a job's staging directory is only deleted once its *full* pipeline (download → organize → scan) succeeds — a failed job (restart, crash, or any other mid-run death) leaves its downloaded files on disk. `retryJob()` resumes that same job id (and therefore the same staging dir, since `pluginStagingDir(pluginId, jobId)` is deterministic) instead of starting a brand-new job with an empty directory, and the spotdl plugin passes `--overwrite skip` so already-downloaded tracks aren't re-fetched. This turns a truncated 893-track retry from a full re-download into "pick up the ~200 remaining tracks." Scoped to spotdl only; yt-dlp/archive keep their existing retry-from-scratch behavior. Staging dirs are swept on success, on manual `deleteJob`, and by the 7-day stale-job janitor, so nothing accumulates indefinitely.
