...
Re-run across the variety switcher, the shipped lexical axis returns **1
distinct value** across the served 15 on `balanced` and `similar` (everything
ties at 1.00) and **3** on `different` — it has only 1.0 / 0.6 / 0 to give. The
learned axis returns 5 / 3 / 9. `different` keeps its 30 % out-of-genre quota
either way (it is pool-and-quota, not score-driven) but can finally *order* it.
Details in the measurement doc; `different`'s genre weight of 8 was set against
a three-valued axis and is worth revisiting at the v9 flip.
