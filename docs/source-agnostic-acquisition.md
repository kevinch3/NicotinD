...
> The page/nav rename landed: it is now **"Acquire"** at route **`/acquire`** (renamed from `/search`), with a query-param-preserving `{ path: 'search', redirectTo: 'acquire' }` redirect so existing links/bookmarks/e2e `goto('/search')` still resolve. The component keeps its `SearchComponent` name because the backend is still `/api/search`. Still **left open on #227** (product): whether a lightweight library-find box belongs on the Library page.
