...
> Open product decisions deliberately **left for #227** (implemented the safe structural subset only): renaming the page/nav label ("Get music"/"Acquire") and the `/search` route path, and whether a lightweight library-find box belongs on the Library page. The route + "Search" label are unchanged for now.
