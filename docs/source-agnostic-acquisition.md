...
`GET /api/search?q=` queries the local library first (`LibrarySearchProvider`) and fires the slskd network search in parallel. The API still returns `local` results (a listener's search is library-only), but the **web Search page is acquisition-only** (issue #227) — it no longer renders `local` (no "In your library"/"Songs" sections). "Find what I own" moved fully to the Library tabs/filters + Radio; Search reads unambiguously as "find and add new music". A non-acquirer (listener, or acquisition off deployment-wide per #235) sees a "browse your Library instead" empty state (`data-testid="search-acquisition-off"`). This doesn't conflict with the north star: the blended list is about _sources of acquisition_, and dropping owned-library rows keeps it purely about acquirable candidates.

> **Superseded by the `/get` merge.** Acquire and Downloads are now one nav item (labelled "Add") at route **`/get`**, with acquisition search on its `?tab=find` pane; `/search`, `/acquire` and `/downloads` are function `redirectTo`s that carry incoming params and add the right `tab`, so every existing link/bookmark/e2e `goto` still resolves. The component keeps its `SearchComponent` name because the backend is still `/api/search`. **#227's open product question is now answered**: the "find what I own" half got a real home as the Library **cross-type find bar** (albums + artists + songs in one box), which is where the `local` lane this route still returns is actually rendered. See [web-ui.md](web-ui.md) "The /get workspace" + "Library find bar".
