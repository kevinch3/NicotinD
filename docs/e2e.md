...
- **A fresh service worker holds every fetch until its whole prefetch lands —
  ...
  the **suite-wide default** for the `chromium`, `onboarding` and `tv` projects
  rather than a per-spec opt-in: almost nothing under test is *about* the SW, and every
