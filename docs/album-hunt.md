...
**A failed lookup is not an empty discography (#665).** A swallowed `album.lookup` failure used to render the same §A6 banner as "artist has no albums" — on prod that turned a 20s Lidarr metadata blip into "we couldn't load One Direction's albums" plus a page of raw peer noise. `search()` now runs the album lookup through `albumLookupWithRetry`: one bounded retry on a *fast* failure (5xx/connection refused), **never on `LidarrTimeoutError`** (the 20s lookup budget is already burned out of the web's 30s GET abort window), and sets `albumLookupFailed` on the result when it still fails. The web's `discographyFallbackNote` returns a structured note — `no-albums` keeps the discography CTA; `lookup-failed` wins over it and swaps in a **Retry** button (`retryCatalog()`, `data-testid="retry-catalog"`) that re-runs only the catalog lookup, while `canLoadDiscography` hides the Lidarr-mutating CTA during the outage (an answer we never got is no basis for a provision).
...
**One-click Get requires — and now sends — the addon `candidateRef` (issue #530).** Since the addon cutover, `hunt-download` resolves the pick addon-side by the hunt candidate token and 400s ("Selection expired — run the search again") without it. The manual modal was migrated; `AutoHuntService` (the "Get" button path nearly every hunt takes) was not, so **every** one-click download failed on prod while manual worked — and the failure was invisible because both layers flattened the reason (below). The service now sends `candidateRef` and **self-heals at enqueue time**: on a *retriable* failure (5xx / network — `classifyHuntDownloadError` grew a `retriable` flag; 4xx is terminal since a stale selection fails identically for every peer) it advances to the next candidate with `matchPct ≥ 60`, bounded at **3 total attempts** (`MAX_DOWNLOAD_ATTEMPTS`) so a systemic failure (addon down) fails fast instead of burning the whole list, narrating each hop with an info toast. This complements — not duplicates — the addon-side cross-peer fallback, which only covers failures *after* a job exists. **Failure reasons surface end-to-end (issue #531)**: the classifier now carries the server body's human message (it used to flatten everything non-machine-code to "Download failed"), the final error toast includes it (with a "Find Manually" action), and every `hunt-download` failure branch `log.warn`s server-side — the 502 body also appends the addon's own error after the "they may be offline" guess. The whole #530 incident produced zero log lines and an unactionable toast; either layer surfacing "Selection expired" would have made it self-diagnosing.
