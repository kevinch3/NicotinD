...
- **Tile interaction standard (#1298)**: every tile and row answers the same three gestures with the same menu. **Hover** fades in a ⋯ (`EntityMenuButtonComponent`, `data-testid="entity-menu-button"`, `opacity-0 group-hover:opacity-100 focus-visible:opacity-100`, so it is in the tab order), **right-click** and a **touch hold** (≈450ms still; a mouse press is never a hold — desktop has hover and right-click) open it at the pointer (`EntityActionsDirective`, `[appEntityActions]="() => TrackAction[]"`, a factory so a grid builds a menu only when one is asked for; the hold eats the release's click through a capture-phase listener so the tile does not also open). There is **one open menu**, `EntityMenuHostComponent`, mounted once in the layout like the confirm host: tiles call `EntityMenuService.open({ actions, at | anchor })` and never carry a panel of their own; it is clamped into the viewport above the bottom chrome (`clampMenuPosition` + `measureBottomChromeInset`), closed by Escape, an outside click, hardware Back or running an item, with ArrowUp/Down between items. **One action source per kind**: songs keep `SongMenuService.build()`, albums/artists/genres/playlists get `EntityMenuService.build(ref, ctx)` in a fixed order — Start radio (the app's one verb), Play, Play next, Add to queue, Save offline, Open, then `ctx.extraActions` last (the Library's curator Hide/Unhide album rides there) — every verb fetching the entity's tracks through the same API the detail page uses and handing them to the same `PlayerService` calls, with the detail pages' own preserve keys. A genre radio is a filter radio (`getFilterRadio` → `startRadioWithFilter`), the rest are list radios anchored on their own songs. Wired on: the Library album/compilation/single/artist/genre cards and playlist rows, `album-tile` (owned/partial only), the four classic shelves (song tiles draw the song menu, tastemakers the playlist menu), `track-row` (right-click and hold open its existing ⋯ list), and the mosaic (delegated on the stage: the old hold-for-info is now the hold-for-menu with **Song info** as an item, right-click on a tile, and a `.mosaic-more` ⋯ in each face's HTML). The menu services are resolved through `Injector.get` at open time so a page rendering a hundred tiles never instantiates their dependency graph. Nothing on TV (`isTvUi()`): a D-pad long-press is a different contract and there is no hover. **Haptics are not here** — `@capacitor/haptics` is not a dependency; #1297 carries it. Covered by `entity-menu.service.spec.ts` (order, verbs, keys, failure toasts), `entity-actions.directive.spec.ts` (fake-timer hold, slop, cancel, click suppression, TV), `entity-menu-host.component.spec.ts`, `entity-menu-button.component.spec.ts`, the wiring specs on library/track-row/album-tile/mosaic, and `entity-menu.spec.ts` (e2e: right-click and hover on the Library, a track row, and the mosaic hold → Song info). The touch hold is a real-device gate, as for the player swipes.
- **Per-user preferences (#1299)**: what follows a person across devices — `homeView`, `theme`, `followSystemTheme`, `language`, `radioStrategy`, `welcomeDismissed`, `queueAcquired` (the get-then-hear opt-out, [get-then-hear.md](get-then-hear.md); `null` reads as on, and a mirror without the key still parses) — is one typed object, `UserPreferences` in `@nicotind/core` (every key nullable on read, `null` = never chosen on the server; a patch never carries null and must choose something). **The zod schemas (`UserPreferencesSchema`/`UserPreferencesPatchSchema`, `user-preferences-schema.ts`) are API-only**: the web validates its mirror and server replies with the hand-written `parseUserPreferences` from the zod-free `user-preferences.ts`, because importing a zod schema into a root service put zod into the initial bundle and blew the 780 kB budget by 57 kB. Server: `user_settings` columns (`home_view`, `follow_system_theme`, `language`, `queue_acquired` added; the legacy `theme` column's NOT NULL `'system'` default reads as null), `services/user-preferences.ts` `getUserPreferences`/`patchUserPreferences` (a merge upsert), `GET`/`PATCH /api/me/preferences` (`routes/user-preferences.ts`, caller-scoped, 400 + `VALIDATION_ERROR` on an unknown key or out-of-range value) and the same object **embedded in `GET /api/auth/me`** so boot needs no second round-trip. The older per-key routes (`/auth/dismiss-welcome`, `/recommendations/preferences`) write the same columns and stay as aliases. **The three dead columns (`autoplay_on_load`, `default_min_bitrate`, `default_file_types`) are left in place**: the schema is additive-only and `config-export.ts` / `privacy.ts` read `user_settings` generically. Web: `UserPreferencesService` is the one door — a **per-device mirror** (`nicotind-prefs`) read synchronously at construction so the first paint after a reload already shows the remembered choice, `hydrate()` from `/me` (server wins), and `patch()` that is optimistic, reverted on a failed write, and **sends nothing without a stored session** (the login/setup/share pages carry a language picker before any user exists, and a 401 there would make the interceptor bounce the user mid-login). `HttpClient` is optional so specs that never provide it still run. The owners keep their own signals and device keys and depend on the door, never the reverse: `ThemeService.setTheme`/`setSystemTheme` and `TranslateService.use()` (a *choice*, not the init-time resolution) write through, `adoptPreferences()` on each takes what the door holds after `refreshSession` hydrates it, and both consult the mirror at construction/init so the last person to sign in on this device wins over the device key. `AuthService.resetSession()` clears the mirror. Covered by `user-preferences.service.spec.ts`, the theme/translate/app.config specs, the API route + service tests, and `theme.spec.ts` (e2e: choose, wipe the device keys, reload, still chosen).
...
- **Player expand/collapse gesture**: one vertical-swipe primitive, `createVerticalSwipe()`
  (`packages/web/src/app/lib/vertical-swipe.ts`, composed on `createPointerDrag()`), serves the
  mini bar and the Now Playing sheet. It arms a non-passive `touchmove` blocker at pointerdown and
  asks the call site's `resolve(ctx)` exactly once — from whichever of the blocker or the pointermove
  stream crosses `SWIPE_SLOP_PX` first — so the blocker can never prevent a move the handler would
  release. That is the pull-to-refresh lesson (#731) applied to the player: the mini-bar body was
  bound to the open gesture but carried `touch-pan-y`, so the browser reclaimed the pan after ~10px
  and only the `touch-none` grab notch (`data-testid="player-grab"`) ever opened the sheet. The body
  is `touch-none` now and the swipe is **live-follow**: the closed sheet rides
  `PlayerService.nowPlayingLiftPx` up with the finger (`translateY(calc(100% - lift))`) and commits
  on end **or cancel** via `shouldCommit()` — distance past `OPEN_THRESHOLD_PX` (96, up from the old
  40px commit-on-move, because a sheet that visibly follows makes 40px read as accidental) *or* a
  flick ≥ `FLICK_PX_PER_MS` measured by `flickVelocity()` over the trailing 100ms. A tap still opens.
  The sheet (`now-playing.component`) has **one body gesture** on its root (`onBodyPointerDown`;
  `button`/`a`/form controls/`[data-seek]`/`.seek-range`/the waveform/`app-menu-panel`/
  `[data-np-no-swipe]` are excluded, and nothing starts under karaoke fullscreen). Its mode is
  decided at resolve time from the origin zone (`data-np-zone="panel"` on the tabs+panel wrapper,
  everything else is the stage) and the current state:

  | origin | direction | condition | mode |
  | --- | --- | --- | --- |
  | any | up | below `lg` and the panel has room to grow | **resize** (grow the panel) |
  | any | up | otherwise | release (native scroll) |
  | panel | down | the nested scroller's `scrollTop > 0` (`scrollableAncestorTop`) | release |
  | panel | down | at the top, panel grown | **resize** (collapse) |
  | panel | down | at the top, panel at rest | **dismiss** |
  | stage | down | panel grown, below `lg` | **resize** (collapse), then continue |
  | stage | down | otherwise | **dismiss** |

  **Continuation**: a collapse that runs out of panel flips to `dismiss` mid-gesture (offset
  measured from the point the panel hit 0), so one uninterrupted pull shrinks the panel and then
  drags the sheet down. Dismiss follows the finger (`dragOffsetPx` → `[style.transform]`,
  `transition-none` while dragging or lifting) and commits past `DISMISS_THRESHOLD_PX` (120) or on a
  flick. The header pill and the resize notch are visible affordances only — their pointerdown
  bubbles to the root. Player text uses `translate="no"` + `.no-callout` to suppress the mobile
  translate/selection popup. Artist/album navigation lives only in Now Playing — the mini bar never
...
- **Zoom disabled app-wide** (`styles.css` → `html { touch-action: pan-x pan-y }`, plus `maximum-scale=1, user-scalable=no` in the `index.html` viewport meta): stray double taps and pinches kept zooming the viewport on touch builds (web + native WebView), and iOS zoomed in on focusing any field under 16px (most fields are `text-sm`/`text-xs`) and stayed zoomed after blur. The app has no content that needs zoom, so both gestures are off rather than just double-tap (the earlier `manipulation`). `pan-x pan-y` keeps scrolling; the viewport meta stops pinch in the iOS WKWebView and focus-zoom in iOS Safari too (Safari itself still allows pinch; Apple ignores `user-scalable=no` there). The root rule is an element selector, so the finer-grained gesture surfaces win by specificity: the seek range (`.seek-range { touch-action: none }`) and the player drag handles (Tailwind `touch-pan-y`/`touch-none` utilities). Asserted in `mobile-ux.spec.ts` ("zoom is disabled via root touch-action and the viewport meta").
...
- **Now Playing — component split + tabbed Queue/Lyrics panel** (player-standardization plan,
  ...
  **JIT vitest harness limitation (read before adding a shell-level test that clicks a child)**: the
  ...
  calls it.
  - **Manual queue resize** (`data-testid="now-playing-queue-resize"`): the notch is the visible affordance for a resize the sheet's body gesture serves from **anywhere** — swipe up on the cover, transport, tabs or panel to pull the panel area **taller**, which **shrinks the cover art** to make room (the panel is `flex-1`, so shrinking the cover grows it); swipe down to collapse it (see the mode table above). The handle is **shell-owned, rendered above the Queue/Lyrics tab bar** — it originally lived inside the queue panel, so after the tab switcher shipped it vanished on the Lyrics tab and users reported the feature as lost; hoisting it makes it work for **both** panels (the queue panel's old `resizing` input / `resizeStart` output API is removed). `queueExtraHeightPx` (px the cover has shrunk from its 320px max, clamped to `[0, 320]` — the floor is 0 since #993) feeds `coverMaxPx = 320 − queueExtraHeightPx`, bound to the cover's `max-width`. The chosen size is **persisted per-device** in `localStorage` (`nicotind:np-queue-extra`) and read on construction, so it survives reload. Unit-tested in `now-playing.component.spec.ts` → "sheet body gesture (mode table)", "queue resize (drag handle)" + "hoisted resize handle (shell-owned)"; the Lyrics-tab drag and the swipe-from-the-tabs path are covered end-to-end in `mobile-ux.spec.ts`.
  - **Desktop side panel (`lg:` ≥1024px, Spotify-like)**: the sheet becomes two columns — cover art + transport centered in a flexible left column, the Queue/Lyrics panel (with its tab switcher) an always-visible right column (`data-testid="now-playing-body"` wraps both). **The border between them is a splitter** (`data-testid="now-playing-side-resize"`, `role="separator" aria-orientation="vertical"`, `tabindex="0"`, 8px hit area around a 1px line): drag it, or with focus use ArrowLeft/ArrowRight (±16px) and Home/End (min/max); double-click resets. `sidePanelWidthPx` defaults to 380, clamps to `[300, 640]` and never more than half the viewport, and is **persisted per-device** (`nicotind:np-side-width`). It rides a **`--np-side-w` CSS var** (`lg:w-[var(--np-side-w,380px)]`) rather than an inline width, for the same reason the cover cap does. Because every extracted child uses `host: {class: 'contents'}`, a grid/flex on the root would place each child's *inner* element as an individual item — so the shell adds two **group wrappers that are `contents` below `lg` and real flex columns at `lg`** (`class="contents lg:flex …"`): below the breakpoint the wrappers dissolve and the mobile stacked layout is untouched by construction. The breakpoint is `lg`, not `md` (the app's nav-chrome fork) — a portrait tablet at 768px lacks room for cover + a 380px column, and `lg:` is where the app's grids already assume real horizontal space. The mobile drag-resize notch is `lg:hidden` and the body gesture's resize mode is skipped at `lg` (`matchMedia('(min-width: 1024px)')`); the stored cover shrink is mobile-scoped: `coverMaxPx` rides a **`--np-cover-max` CSS var** (`max-w-[var(--np-cover-max,20rem)] lg:max-w-80`) instead of an inline `style.max-width`, which would beat every class including the `lg:` cap. Karaoke fullscreen is untouched (own `fixed inset-0 z-[70]` overlay at all widths). Covered by `now-playing-desktop.spec.ts` (e2e; jsdom has no layout engine) + the cover-art spec's var-binding case.
...
- **Playback auto-radio**: `PlayerService.radio` (persisted in the player state snapshot) keeps playback going — a constructor `effect()` watching `queue().length` calls `replenishRadio()` whenever the queue sits below `radioQueueTarget` (and `repeat==='off'`), appending fresh tracks (de-duped against current/queue/recent history). It asks for the shortfall, not a fixed batch, so the queue holds one depth as it is consumed rather than draining and refilling in lumps — see [radio.md](radio.md) "A radio queue has a depth, not a batch size". The `RadioProvider` registered by `RadioSourceService` calls `GET /api/radio/next` with the session's anchor as seed (`PlayerService.radioAnchor`: the song or list the radio was started from, never the playing track — see [radio.md](radio.md) "A radio is about its anchor", #1277) — the server scores candidates by BPM, key (Camelot), genre, year, duration, and artist diversity, returning musically similar tracks. Falls back to shuffled recent songs when there is nothing to anchor on yet. `PlayerService` stays HTTP-free via the `RadioProvider` callback. Toggle lives in the Now Playing sheet. **Filter "vibe" radio**: `PlayerService.radioFilter` (a persisted `LibraryFilter`, set by `startRadioWithFilter(tracks, filter)`) makes the provider replenish via `getFilterRadio(filter,…)` so a mood/genre/bpm radio stays in-vibe; it's cleared when seed radio starts or radio turns off. → [docs/radio.md](../docs/radio.md)
- **Home view switch (`pages/home/`, the home route `''`, #1300)**: the route is a thin `HomeComponent` shell that renders the view the user chose — the mosaic (default) or the classic shelves — and a *Shelves | Mosaic* `HomeViewSwitchComponent` (`role="radiogroup"`, `data-testid="home-view-switch"` / `home-view-mosaic` / `home-view-shelves`, 44px targets, styled like the Library tabs) in the **top-left corner**: floating over the field on the mosaic (the header is hidden below `md` there, so that corner is the screen's), in flow above the first block on the shelves. The choice is `homeView` on the per-user preferences door (see "Per-user preferences"), so it follows the user across devices and the per-device mirror makes the first paint after a reload already the chosen view. **Only the chosen view's chunk is loaded**: `HOME_VIEW_LOADERS` is one `lazy()` import per view, resolved in an effect on `view()` and rendered through `NgComponentOutlet`; the other page is never fetched and its data (`getRecentPlays`, `getListRadio`, `getRandomSongs`, `getStats`) never requested until the user switches, and switching swaps the component in place — same URL, no navigation. The shell's host is `contents` so the mosaic's `absolute inset-0` stage still positions against `<main>`. `headerDisplayClass(url, homeView)` hides the top bar below `md` only for the mosaic — with the shelves the home is a scrolling page again and gets its sticky header back. `/classic` stays as a direct link to the same shelves component and does not touch the preference. `homeViewOf(null)` is `'mosaic'`, so nobody who never chose sees a change. TV has its own home (`tvShellRoute()`). Covered by `home.component.spec.ts` (the unchosen loader is never called; a switch patches the door and swaps in place), `home-view-switch.component.spec.ts`, the `headerDisplayClass` cases, and `home-view.spec.ts` (e2e: switch, server-side persistence, reload, no `/api/history/stats` request while on the shelves, header back on a phone).
- **Mosaic home (`pages/mosaic-home/`, rendered by the home shell for the default `mosaic` view)**: the post-login surface — one infinite, pannable field of tiles where **every tile starts a radio**. It is composed from the sources the classic landing uses (keep the vibe, taste breakers, recently played, curated playlists, vibe presets, top genres — not Resume, see "The recipe is a draw" below), collapsed into one surface with one verb. The classic landing used four playback verbs depending on which shelf a cover sat in (`startRadio`, `playWithContext`, `startRadioWithTracks`, `startRadioWithFilter`); here `MosaicAction` has three shapes and all three are radio starts. Recently-played is the one source whose behaviour changed: it no longer uses `playWithContext`, so tapping it no longer keeps the shelf as a queue.
  ...
  **The plane is a torus.** Panning never runs out of content because the packed patch repeats every `W` in both axes; the renderer draws copies at `x + i*W, y + j*W`. That only works if collisions are tested with *wrapped* distance, so a tile near the right edge knows about one near the left — they are neighbours once the patch repeats. Two constants that a demo can hardcode must be derived here: `patchSide()` sizes `W` from total tile area (a fixed side leaves a 12-tile library packed into one corner with nothing to pan through), and `cellCount()` keeps the collision hash's cell at least one max-tile-plus-gap wide, because the `fits` test only inspects the 3×3 cell neighbourhood and a smaller cell silently misses collisions.

  **A torus has no centre, so the packing must not have one either.** The scan order was once centre-out — candidates sorted by distance from the patch middle, first fit wins. First-fit from a centre packs an *inscribed disc* and leaves the square's corners empty, and tiling that patch merges four empty corners into one void every `W` pixels: the plane visibly repeated as blobs separated by channels. Measured over the real tile mix, the old ordering put **zero** tiles outside the inscribed disc at every tile count and left channels 30–120px wide; it also degraded badly with size, dropping all eight of its largest tiles into a *single* block of nine at 120 tiles. `positionRank` replaces it with a fixed hash permutation — centre-free, and still deterministic so a tile does not move between reloads — and `CANDIDATE_SAMPLES` adds Mitchell's best-candidate: of the first few positions that fit, take the one farthest from its nearest neighbour. Random alone would remove the void but clump; the spacing rule is what makes the field *even*. Two consequences worth knowing:

  - **Order became largest-first.** The old alternation (biggest, smallest, second-biggest…) existed to stop big tiles clumping centrally, which was an artefact of the centre-out scan. Once spacing spreads them by construction, order is free to serve packing instead — and it must: spreading tiles evenly fragments the free space, so interleaving sizes strands the largest tiles late and the packer **silently drops** them (81 tiles became 74). A big tile needs a big gap and gaps only shrink, so the biggest goes first. `packMosaic` is spec'd across 1–120 tiles because 40 packed fine while 81 lost seven.
  - **It is ~16× faster.** The old ordering sorted ~163k candidates with `Math.hypot` on every comparison, costing 3.5s per pack at a desktop stage (21.7s at 200 tiles) — paid on load *and on every resize tick*. An integer hash sort brings the same case to 224ms.

  `mosaic-packing.spec.ts` asserts the two properties that make the field continuous — tiles reach the patch corners, and no empty channel crosses it — rather than the mechanism. The test they replaced asserted that placement *order* interleaved sizes, and passed happily throughout the clumping above.

  **The geometry repeats; the content does not.** Repeating the home patch's *content* made exploring feel like a loop — pan far enough and the same eighty tiles came round again. So only cell `(0,0)` shows the curated home tiles; every other torus cell fills the same slot geometry with its own fresh batch, fetched from `GET /api/library/random` the first time the cell is sighted (`enterCell` → `loadCell`), scored with the caller's own play weights and mapped best-score-into-biggest-slot by the pure `assignSongsToSlots`, so tile size keeps meaning "hotter" out in the field. While a batch is in flight the cell's slots wear a pulsing `.skeleton-block` face; a failed or empty fetch degrades that cell to the home copy (`'base'`) rather than a hole — and deliberately never retries until the cell is evicted, so a down server cannot become a fetch loop. Cells are cached (capped at `MAX_CACHED_CELLS`, stale-evicted by frame stamp) so panning back shows what you saw; an evicted cell simply refetches, which for a random batch loses nothing. Newly surfaced tiles are mirrored into the hidden accessible list (`discovered`, capped).

  **The lens** is an orthographic azimuthal projection: distance becomes an angle (`θ = r / R`), screen radius is `R·sin θ` and scale is `cos θ`, so tiles bunch and shrink toward the rim instead of marching off the edge at constant size. Past `THETA_MAX` the surface has turned away and is culled.

  **The recipe is a draw, not a fill (`LANE_MIX`).** The first cut filled every lane to the brim — twenty recent plays, ten keep-the-vibe variations, twenty-four random picks — and real use showed the cost: more than half the song tiles were the listener's last session and its few artists (one album listen is up to twenty tiles of one artist, since `recentPlays` collapses per *recording*, never per artist), and because history barely moves between two mornings the wall was the same on every visit. `LANE_MIX` in `lib/mosaic-tiles.ts` is now the second hand-tuned block beside `SCORE_WEIGHTS`. Each lane still fetches its whole pool — `recent.pool` is twenty plays, `keepVibe.pool` twenty variations seeded by *all* twenty plays, drawn or not — but shows only `tiles` of it (ten each, half), drawn uniformly without replacement by `drawFrom`, a partial Fisher–Yates with an injectable rng so the spec can seed it. So each recent play has even odds of being on the wall today, no two visits compose the same wall, and taste breakers fill the rest of `songSlots` (54, the old total), skipping anything either pool holds — a play that lost its draw must not walk back in as "random", or the caps would be a fiction. With no history at all the breakers take the whole budget, which is why they are fetched at `songSlots`: a fresh install still gets a full field. **The Resume tile is gone**: the mini-player already is the one-tap continue, and a fixed score-1 tile of the track you are already hearing was the largest thing on the wall on every visit. The classic landing keeps its Resume card. Two things this deliberately leaves alone, for the record: `GET /api/library/random` is uniform over *songs*, so a big catalog still wins the breakers and every discovery cell in proportion to its track count; and `ownPlayShare` still sizes any song by an all-time top artist near the maximum. If one catalog keeps recurring, the next step is an artist-stratified draw plus a per-artist cap, not a smaller pool.

  **Tile size is a ranking.** `scoreTile` blends `Song.popularity` with the caller's own play counts from `GET /api/history/stats`. Popularity is absent for most rows (mid-backfill, and ~half the library has no recording-MBID tag to score against), so an unknown value **redistributes its weight** onto plays plus a stable per-key hash rather than substituting a constant — a `?? 0.5` default would give almost every tile the same size and flatten the mosaic into a uniform grid. `SCORE_WEIGHTS` is the one block meant to be tuned by hand.

  **Tap versus drag** is a 6px slop test on accumulated pointer travel, checked in a single delegated `click` listener on the stage. Without it every attempt to pan ends in a pointerup on a tile and starts a radio; delegation (rather than a handler per element) is required because the pool recycles elements out from under any handler bound to one. Pull-to-refresh needs no opt-out: the layout gesture is gated on `PullToRefreshService.hasHandler()` and this page registers no handler.

  **A release comes to an actual stop.** Inertia decays by `FRICTION` per frame with a hard snap to zero below `REST_EPSILON` (`decayVelocity` in `lib/mosaic-lens.ts`) — exponential decay alone never reaches zero, so the camera crept sub-pixel forever and read as "it never stops". The demo's idle drift was removed for the same reason: a resting mosaic is genuinely at rest.

  **Press and hold a song tile** (450ms within the tap slop) opens the shared track-info sheet via `TrackInfoService` — album, artist, genres, mood, stats and the like toggle, all the sheet already carries — with a suppress flag eating the click that follows the release so the hold does not also start a radio under the sheet. Only song-backed tiles (`song`) arm the timer. Hovering with a fine pointer lifts the face slightly: faces rest at `scale(0.97)` and grow to `1` on hover, because `contain: strict` clips paint at the tile box so a scale past 1 would crop, not lift — the same reason `.mosaic-pop`'s fill is `backwards`, letting the animation release the transform once the pop lands.

  **Offline, the mosaic becomes the downloaded set.** Every source it draws from is a server read, so losing the network mid-session used to leave a wall of dead tiles — and the app worked around that by *navigating away*: `/` was the one entry in the shell's `ONLINE_ONLY_ROUTES`, `onlineOnly` in the bottom tab bar, and `app.ts` actively redirected to `/library` whenever `isOffline()` flipped. All three are gone. `buildOfflineTiles` fills the field from `PreserveService.preservedTracks` — IndexedDB, no token, no request — and the swap runs in both directions, so the page a listener is on survives a tunnel and comes back when the signal does. Four things this settles:

  - **Radio is not offered, because it cannot work.** Every radio provider fetches its next tracks from the server, so a tile promising radio here would fail on tap. The offline action is `playWithContext` over the whole downloaded set under the `saved-offline` context — the same verb the Library's offline Songs tab uses, so playback runs on through the device's music instead of stopping after one track. This is the documented exception to "every tile starts a radio", and `mosaic-tiles.spec.ts` pins it: offline, every tile's action is `offline` or `route`.
  - **One tile navigates rather than plays.** The `downloads` tile routes to `/library`, which force-selects its Songs tab offline and so *is* the downloads list; `/get?tab=downloads` is the acquisition feed and needs the server. It skips the playback busy-guard and never raises Now Playing.
  - **Covers are omitted, not requested.** A cover id would build an `/api/cover/:id` URL that cannot resolve, and a face with no `coverArt` already falls back to its deterministic placeholder gradient. Reading cover blobs out of IndexedDB per tile would put object-URL lifetimes inside a recycling pool, which is where leaks live.
  - **Discovery cells stand down.** `enterCell` returns `'base'` immediately while offline rather than firing one request per sighted cell that can only fail.

  Sizing falls back to what the device itself knows — how recently each track was downloaded, plus the usual stable jitter — since popularity and play stats are both server reads. The reload effect tracks `NetworkStatusService.reconnects` *alongside* `isOffline`, because signals coalesce: a fast offline→online pair flushes one run carrying only the final value, so the edge is invisible to a diff of `isOffline` alone. A load token guards the swap, so an online load started before the drop cannot land on top of the offline set that replaced it.

  **Loading is shape-matched.** The initial fetch renders the `mosaic` variant of `SkeletonComponent` — a deterministic scatter of pulsing squares where the field will appear (`mosaicSkeletonSpot`), not a list skeleton borrowed from another page. On the home route the layout also collapses the top bar below `md` (`headerDisplayClass` in `layout.component.ts`): every control in it is `hidden md:*` there anyway — nav lives in the bottom tab bar — so on a phone it was a dead brand row eating a strip of an immersive, full-bleed surface. Every other route keeps the sticky header.

  **Pooled tiles cannot hold a focus order**, so the page also renders a visually-hidden `<ul>` of the same tiles as real buttons calling the same `start(tile)`. That list is a plain Angular `@for`, which is also what the e2e spec drives — pooled tiles are recycled mid-interaction and are a flaky click target. Tile markup and `.mosaic-*` styles live in global CSS (`styles.css`), since elements created by the loop carry no view-encapsulation attribute; `contain: strict` on the stage and each tile is what keeps the field at one paint per frame. The loop writes transforms and never a signal, and the app is zoneless, so it costs no change detection and needs no `runOutsideAngular`. `prefers-reduced-motion` disables the pop-in and the hover transition. `data-testid`s: `mosaic-home`/`mosaic-stage`/`mosaic-tile`/`mosaic-tile-list`/`mosaic-tile-link`. → [docs/radio.md](../docs/radio.md)

- **Classic radio landing (`pages/radio-landing/`, route `/classic`, and the home shell's `shelves` view)**: the shelf-based landing the mosaic replaced; since #1300 a user can make it their home with the switch above, and it stays reachable at `/classic` as a direct link. Built to *start listening in one tap* — recover a mood or make a new one. Six blocks: (1) **Resume** — a card seeded from the persisted last track (`PlayerService.currentTrack`); tapping `startRadio`s it and the block **disappears** (a local `resumeDismissed` signal; hidden entirely when there's no last track). (2) **Keep the vibe**, (3) **Taste breakers**, (4) **Recently played** and (5) **Tastemakers** — four self-hiding shelves, all documented in [docs/radio.md](../docs/radio.md); the page reads the recently-played rows off the child via `viewChild` and feeds them to both (2) and (3), so the history is fetched exactly once. (6) **Start a radio** — one-tap **vibe presets** (Happy/Chill/Party/Energetic/Danceable/Uplifting/120bpm+/Acoustic, each a canonical `LibraryFilter` over `MOOD_VOCAB`/perceptual buckets/bpm) + top-genre tiles. Each tap calls `getFilterRadio` and hands the result to `startRadioWithFilter` (empty match → a neutral toast, never an error).

  **The two tones are the design** (`components/vibe-tile/`). One `VibeTileComponent` renders both rows so they cannot drift: `tone="color"` + `[wide]="true"` is a vibe (its own fixed Tailwind gradient under bold white text, `w-40`), `tone="muted"` + `[wide]="false"` is a genre (flat `bg-theme-surface-2/70`, small `text-theme-secondary`, `w-[4.75rem]`) — recessive on purpose, so the genre index never competes with the vibes. The gradients are **fixed** colour pairs rather than `--theme-*` tokens: a theme-derived gradient collapses to grey under OLED and Daylight, erasing the contrast the two tones exist to create; white-on-gradient is legible on all six themes without a per-theme accent shadow. The vibe row is `grid grid-flow-col grid-rows-2` + `overflow-x-auto`, so it is a fixed two rows that scrolls sideways at every width instead of growing tall on a phone.

  The tile's `data-testid` sits on the **host** `<app-vibe-tile>`, not the inner `<button>` — a signal input can't carry it, because the JIT vitest harness never registers inputs on a nested component (`testing/signal-input.ts`, landmine 1) — and the host carries `class="inline-flex"` so it has a real layout box for a Playwright click. That same limitation is why the tone/width contract is pinned in `vibe-tile.component.spec.ts` (component as root) and `e2e/tests/radio-landing.spec.ts` (real build, now driving `/classic`), **not** in `radio-landing.component.spec.ts`, where every nested tile renders its defaults. `data-testid`s: `radio-landing`/`radio-resume`/`radio-resume-play`/`radio-preset`/`radio-genre`/`taste-breakers`/`taste-breakers-item`. → [docs/radio.md](../docs/radio.md)
...
- No matching of its own. `/api/search`'s local lane (`LibrarySearchProvider`) already tokenizes, NFD-folds diacritics, ANDs per token over a `name+artist` haystack, — it is precisely the matcher the #7 fix installed.
...
`startRadio(track)` clears the queue too, and anchors the session on that track: a leftover queue played out in full before the radio the user asked for ever started. `playSingle` and `playWithContext` clear the anchor (a gesture re-anchors; a radio advance never does). `toggleRadio`'s eager fill covers the now-empty queue. `startRadioWithFilter` already set its own queue and is unchanged.
...
| `mosaic` | `block absolute inset-0 overflow-hidden` | 12 | mosaic home |

`mosaic` is the one variant with no list to mirror: its "list" is a free canvas,
so it scatters squares from a fixed table (`mosaicSkeletonSpot` — deterministic
for the same reason `skeletonBarWidth` is) where the packed field will appear,
percent-positioned with a clamped width so one table serves phone and desktop.
...
## Shuffle and repeat are not in the UI (#1263)
...
which already toggles radio (anchored on the current track when turned on over a bare queue).
...
## Transport buttons are sized for a thumb (#1263)
