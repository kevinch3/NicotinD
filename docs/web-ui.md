...
- **Focus zoom disabled on touch** (`styles.css` → unlayered `@media (pointer: coarse)` rule lifting text fields to `max(16px, 1em)`): iOS Safari and WKWebView zoom the viewport in on focusing any field whose font is under 16px and leave it zoomed after blur, and most fields here are `text-sm`/`text-xs`. It is unlayered so it beats the Tailwind utilities (which live in `@layer utilities`); checkboxes, radios, ranges and colour pickers are excluded. Fine pointers keep the compact size. This, not a viewport `maximum-scale=1`, because that also blocks pinch-zoom on Android. Asserted in `mobile-ux.spec.ts` ("focus zoom on touch").
