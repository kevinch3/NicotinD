...
- **`POST /api/library/albums/:id/cover`** (`ApplyCoverRequest`): exactly one of —
  ...
  - `songId` (an album track) → `extractEmbeddedPicture` → `replaceFolderCover(dirname)` → **`deleteArtwork` (clears the canonical override) + `purgeDiskArtCache`** so the cover route falls back to the new folder image. This is also the **revert-to-original** path: pick a track whose embedded art is the original to undo a bad canonical cover.
  - Both branches, plus `applyMetadataFix`'s `coverUrl` path, `optimizeAlbum`'s Lidarr-match cover write and the loose-single enrichment, call `clearAlbumCoverNegativeCache(db, id)` (`routes/streaming.js`) after the write. It evicts the album id **and every song id of the album**, because a song id resolves to its album's cover and was left 404ing for up to 10 min when only the album id was evicted (#1336). **Why:** an album with no art yet gets its id memoized in the cover route's `noArtCache` (10 min TTL, see [library-scanner.md](library-scanner.md)) the first time `/api/cover/:id` 404s; every album-cover writer must invalidate that entry for its id or the picked cover silently never appears — not even on a page refresh — until the TTL expires (regression: was only wired for the artist-image override paths, not the album ones; regression-tested in `routes/library.cover.test.ts`).
- **`PUT /api/library/albums/:id/cover`** (multipart, field `image`, JPEG/PNG/WebP ≤ 8 MB — same allow-list/cap as the artist-portrait upload, shared as `MAX_IMAGE_UPLOAD_BYTES`): resolves the album's representative track's folder → converts the upload via `resizeCover(bytes, 1200)` (the same sharp cover-fit-square-WebP function thumbnails already use, just requested at 1200px) → `replaceFolderCover` + `deleteArtwork` + `purgeDiskArtCache` + `clearAlbumCoverNegativeCache`, identical cleanup to the `songId` branch above. 404s if the album has no track files to place a cover next to; 400 if the bytes don't decode as an image.
- **A new folder cover replaces the old one, it does not just sit beside it (#1336).** The upload is always `cover.webp` and a picked track writes `cover.<jpg|png|webp>` by its content type, but every folder-art reader walks `COVER_EXTS` with `.jpg` first. So an album that already had a `cover.jpg` kept serving it: the route answered 200 and nothing changed. `replaceFolderCover` (`services/cover-sources.ts`) writes the new file, then deletes every other `cover.*` (case-insensitively). Other basenames (`folder.jpg`, `front.png`, …) are left alone, because `cover.*` outranks them all. **Nothing is deleted in a shared bucket folder** (`folderArtBelongsToAlbum` false, #978): those images are not this album's, and no reader serves folder art there anyway.
