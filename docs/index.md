...
- **Listener track reports**: anyone listening files into that same queue; `TRACK_REPORT_REASONS`,
  `recordListenerReport`, `curation_flag_reports` as the per-reporter rate limit, `isTasteOnly`
  routing "I don't like it" to the recommender instead. → [mcp-agent.md](mcp-agent.md)
