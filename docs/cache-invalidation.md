...
- **…but they accumulate, so the regenerable ones are pruned on a grace period** (issue #259,
  ...
  *dead* song. It says nothing about songs missing their rows: 580 songs carried a primary in
  ...
  genre-set reader — while this table reported zero orphans for that same join table. The two are
  independent failure modes and only one of them is measured above. The drift is fixed and
  repaired, but the measurement gap it exposed is not. → [genre-model.md](genre-model.md)
...
| `waveform-cache` (`services/waveform-store.ts`, issue #643) | `sha1(path \| mtime \| size \| version)`, 512 MB oldest-first budget | content-addressed like the transcode cache: a replaced file misses, a re-analysed definition is a `WAVEFORM_VERSION` bump. Disk, not DB, so the daily `VACUUM INTO` backup (kept N deep) doesn't multiply ~40 KB × every played track. The route's `noWaveformCache` (id → expiry, 10 min) remembers a failed decode so a corrupt file isn't re-decoded on every Now Playing open (#317 shape) — no explicit eviction: the TTL is short and a repaired re-download changes the content key |
| `stem-cache` (`services/stem-store.ts`, issue #603) | `sha1(path \| mtime \| size \| stem \| model id \| STEM_VERSION)`, 1 GiB oldest-first budget | the separated instrumental FLAC behind the karaoke `?vocals=off`; content-addressed like the transcode cache, plus the model id and a store version so a model swap is a miss, not a migration. Never served directly — the transcode cache derives its `\|stem` variant from it, keyed on the original's identity, so a pruned FLAC just means the next karaoke play re-prepares (basic filter meanwhile). ~25 MB per 3.5-min track, on demand only. Failures live in memory in `VocalSeparationService` (rejected = sticky per file identity, transient = 60 s TTL), never on disk |
