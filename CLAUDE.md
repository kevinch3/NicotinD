...
- **Singles vs album classification**: `LibraryOrganizer` places tracks at `<Artist>/<Album>/<Track>` or `<Artist>/Singles/`; derives album from peer folder name when ID3 tags are missing. → See [docs/download-pipeline.md](docs/download-pipeline.md).
