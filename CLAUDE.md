...
- **Lyrics (on-demand, plugin-sourced, editable)**: new `metadata` plugin kind + `lyrics` capability (LRCLIB first source); stored in `library_lyrics` + file tag, user-editable. The now-playing lyrics toggle opens a karaoke-styled panel (synced line highlighting + auto-scroll) with a fullscreen expand button. → [docs/design-patterns.md](docs/design-patterns.md)
