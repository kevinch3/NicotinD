...
- **Album hunt — soft-ban bypass ("skew search")**: `AlbumHunterService.hunt` (`packages/api/src/services/album-hunter.service.ts`) normally fires `Artist Album` / `Artist - Album` against slskd. slskd/Soulseek silently returns **zero** responses for some exact phrases (a server-side soft ban) even when the files exist. When the client opts in (the "Skew search" checkbox in the album-hunt modal → `skewSearch` on the `POST .../hunt` body) **and** the base queries come back empty, `hunt` retries once with the textually-skewed variants from the exported pure `buildSkewedQueries` (reorder, album-only, drop leading "the", artist + first album word; de-duped and never re-running a base query). Fallback-only + opt-in so a normal hunt gains no noise. The shared search→poll→score body was factored into the private `searchAndScore` so both passes reuse it.
