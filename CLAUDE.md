...
- **Perceptual audio features (no LLM)**: energy/loudness measured bun-side via ffmpeg ebur128; danceability/valence/mood/vocals/acousticness + cached embeddings from the Essentia sidecar (`packages/analysis/`, `NICOTIND_ANALYSIS_URL`); all written to file tags + COALESCE-preserved columns, scored by the Radio engine and sequenced via `energy-arc`. → [docs/audio-ml-enrichment.md](docs/audio-ml-enrichment.md)
...
- **Smart radio (metadata-driven queue)**: `GET /api/radio/next` scores candidates by BPM proximity, Camelot key compatibility, genre match, year, duration, artist diversity, and the perceptual axes (energy/valence/danceability/instrumental/acousticness — 0-neutral when un-analyzed); `PlayerService.radio` auto-appends results when the queue drains. Shared scoring with `/songs/:id/similar`. → [docs/radio.md](docs/radio.md)
