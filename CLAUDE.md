...
- **Album card de-duplication (authoritative, syncer-level)**: the real guarantee that the library shows **one card per album** lives in `NavidromeSyncer` (`packages/api/src/services/navidrome-syncer.ts`), not in Navidrome config. Navidrome fragments one real album into several "albums" two ways the hunt flow routinely triggers, *neither fixable by any `PID.Album` value*: (1) **mixed MBIDs in one folder** — Navidrome keys album identity on `musicbrainz_albumid`, so a folder the hunt/fallback filled with foreign-edition files (deluxe/bonus/acoustic, alternate pressings from per-track recovery) splits into a card per MBID; (2) **punctuation-variant sibling folders** — re-hunts derive the folder name from inconsistent title strings (`¡Bang! ¡Bang!... Estás liquidado` vs `¡Bang! ¡Bang! … Estás liquidado`), producing distinct folders Navidrome sees as distinct albums. The pure helpers in `album-grouping.ts` (`normalizeForGrouping` strips diacritics + punctuation but keeps words; `albumGroupKey` = normalized artist + title; `pickCanonicalId` = fullest fragment, smallest-id tiebreak) drive `mergeAlbums`, which collapses every fragment sharing a group key into one canonical `library_albums` row (id = the fullest fragment) and **remaps all its songs onto that id**; `song_count`/`duration` are recomputed from the remapped songs (not summed across fragments, which would double-count overlapping rips). Genuinely distinct editions stay separate because their *words* differ (`...(Deluxe Edition)`). The UI reads only `library_albums`/`library_songs`, so this is the single source of truth — it works regardless of Navidrome's PID quirks or half-migrated scan state, and existing duplicate rows collapse on the next `syncFull` (which runs at startup and after every scan, so the deploy restart applies it). **Tombstone suppression is group-key-aware**: `library_album_tombstones` gained an `artist` column so a deleted merged album is suppressed (and its tombstone cleared) by `(artist, title)` group key — otherwise a surviving sibling fragment under a non-canonical id would resurrect the album before Navidrome's async scan catches up. The `ND_PID_ALBUM=albumartistid,album` setting (preceding bullet) is now **complementary best-effort** (it reduces how many fragments the syncer must merge) — the syncer is authoritative. **Known limitation / follow-up (not yet done)**: this guarantees one *card* per album but does **not** dedupe *tracks within* a merged album. A canonical card surfaces the union of its fragments' files, so (a) punctuation-variant sibling folders (`¡Bang! …`/`...`) contribute overlapping tracks, and (b) a mixed-MBID folder still physically holds the foreign-edition extras the hunt dropped in (e.g. the `Are You Gonna Go My Way` folder keeps ~34 files for an 11-track album). The disk is untouched. The follow-up is a disk-level consolidation pass: merge punctuation-variant folders into one `<Artist>/<Album>` dir and run cross-folder track de-duplication (extend `album-dedupe.ts`/`repair-album-dupes.ts` to operate across the merge group, keyed on the same `albumGroupKey`), or simply delete + re-hunt the affected albums with the now-fixed flow. Until then, merged cards may list duplicate/extra tracks even though the card count is correct.
