...
- **Auto-preserve queue (PWA lock-screen resilience)**: web-only `AutoPreserveCoordinator` watches the player queue and keeps the next-N tracks as IndexedDB blobs (configurable Off / 5 / 20 / full, per-device localStorage) so playback survives the browser's locked-screen network throttle; `source: 'user' | 'auto'` + `evictAutoLRU` ensures radio churn never evicts user-saved tracks. → [docs/web-ui.md](docs/web-ui.md)
