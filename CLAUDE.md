...
- **Windowed library processing**: resumable background enrichment (bpm/genre/key/energy/audio-features/artist-image) via an extensible task registry, run only inside a daily window; ffmpeg/sidecar failures are diagnosed (stderr tail surfaced, not swallowed as a bare exit code), tallied into `ProcessingStatus`, toasted in the Settings panel (with Run now disabled while running), and reported to Sentry as one aggregated event per failing task. Permanently-broken files (corrupt "Invalid data" decodes) are excluded after N attempts via a `library_song_analysis_failures` ledger (auto-reset on re-download/size-change, surfaced as `skipped`), and every ffmpeg decode has a kill-timeout so one hung file can't wedge a run. → [docs/library-processing.md](docs/library-processing.md)
