...
describe('DownloadWatcher', () => {
  ...
  it('detects a new completed download and triggers a scan', async () => {
    slskdMock.transfers.getDownloads.mockReturnValue(Promise.resolve([
      {
        username: 'user1',
        directories: [
          {
  ...
  it('debounces multiple completions', async () => {
    slskdMock.transfers.getDownloads.mockReturnValue(Promise.resolve([
      {
        username: 'user1',
        directories: [{ directory: 'dir1', files: [{ filename: 'song1.mp3', state: 'Completed, Succeeded' }] }]
      }
    ]));
    ...
    slskdMock.transfers.getDownloads.mockReturnValue(Promise.resolve([
        {
          username: 'user1',
          directories: [{ directory: 'dir1', files: [
              { filename: 'song1.mp3', state: 'Completed, Succeeded' },
