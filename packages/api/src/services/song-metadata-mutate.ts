...
export type SongMetadataMutateResult =
  | {
      ok: true;
      old: { title: string; artist: string; album: string | null; year: number | null };
      applied: AudioTags;
      rescanned: boolean;
    }
  | { ok: false; error: string; status: 400 | 404 | 500 | 503 };
...
export async function mutateSongMetadata(
  ...
  if (deps.scanIncremental) await deps.scanIncremental([relative(md, abs)]);

  return {
    ok: true,
    old: { title: song.title, artist: song.artist, album: song.album, year: song.year },
    applied: tags,
    rescanned: Boolean(deps.scanIncremental),
  };
}
