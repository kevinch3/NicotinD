...
export class DiscographyService {
  ...
  /**
   * Albums with at least one song — i.e. something the user can actually see
   * and play. Presence is graded by songs rather than the album row alone, so
   * the album-name fallback below can't report an empty aggregate (awaiting
   * prune) as present (issue #692 / #687).
   */
      ...
      .query<{ id: string; name: string }, [string]>(
        `SELECT id, name FROM library_albums
          WHERE artist_id = ? AND hidden = 0
            AND EXISTS (SELECT 1 FROM library_songs s WHERE s.album_id = library_albums.id)`,
      )
      .all(artistId);
  }

  /** Every visible song counts as owned — see {@link fetchLocalAlbums}. */
