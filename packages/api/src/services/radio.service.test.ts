...
describe('explainSimilarity (per-axis breakdown — the diagnostic seam)', () => {
  ...
  it('v7 recalibration: a matched acoustic texture outranks a matched runtime (issue #861)', () => {
    ...
    const v6Weights: ScoringWeights = {
      ...
      acousticness: 2,
      embedding: 8,
      // v6 predates the descriptor axes (#642, v8). Pinned at 0 so this
      // comparison stays the v6 one the test name claims — giving them their
      // v8 weights here would silently make it a different measurement.
      timbre: 0,
      groove: 0,
      spectralBalance: 0,
      artistPenalty: 0.15,
 ...
 */
describe('descriptor axes (formula v8, issue #642)', () => {
  ...
  it('ships three composite weights and stamps a formula version of its own', () => {
