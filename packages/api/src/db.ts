...
function applySchemaSteps(db: Database, fromVersion: number): void {
  ...
  // Work / movement (#1369): movements used to be crammed into the title.
  addColumnIfMissing(db, 'library_songs', 'work', 'TEXT');
  addColumnIfMissing(db, 'library_songs', 'movement', 'TEXT');
  addColumnIfMissing(db, 'library_songs', 'movement_number', 'INTEGER');
  ...
  //   v6 — work/movement (#1369): likewise.
  //
  ...
  if (scanCacheVersion?.value !== '6') {
    const now = Date.now();
    db.transaction(() => {
      db.run(`DELETE FROM scan_cache`);
      db.run(
        `INSERT OR REPLACE INTO library_sync_state (key, value, updated_at) VALUES (?, '6', ?)`,
