...
export class PlayerComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    // Audio elements are wired through the vocal-filter graph by the effect in
    // the constructor (it watches audioElA/audioElB signals and attaches each
    // as it resolves). Here we just bind event listeners to the active one.
    const audio = this.audioEl()?.nativeElement;
    if (!audio) return;
