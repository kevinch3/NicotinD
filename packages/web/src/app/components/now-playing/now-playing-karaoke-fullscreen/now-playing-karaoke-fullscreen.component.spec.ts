...
describe('NowPlayingKaraokeFullscreenComponent — vocal mode (issue #1024)', () => {
  // The unit harness renders i18n KEYS (no catalogue is loaded), so labels are
  // asserted by key. Since the ML path was removed the mode is a pure function
  // of the mute flag: `basic` when muted, `off` when not.
  function make(muted: boolean) {
    const fixture = TestBed.createComponent(NowPlayingKaraokeFullscreenComponent);
  ...
  it('muting labels the served mode as the basic center-cancel filter', () => {
    const el = make(true);
    expect(button(el).getAttribute('aria-label')).toBe('nowPlaying.unmuteVocals');
    expect(button(el).getAttribute('data-vocal-mode')).toBe('basic');
    expect(status(el)?.textContent).toContain('nowPlaying.vocalModeBasic');
  });
});
