 ...
 *
 * The impurity is made cheap by memoizing on (key, language, catalog version,
 * params): the per-change-detection work is a string compare and an early
 * return, and the catalog lookup + interpolation only re-runs when something
 * actually changed. The catalog version is part of the key because a memo hit
 * never calls `t()`: without it, a catalog landing after the first render
 * under its language — the normal order at bootstrap, where `init()` is not
 * awaited — left the base text on screen for good.
 */
