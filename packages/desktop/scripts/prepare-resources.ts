 ...
 *   resources/icons/<N>x<N>.png -> paths.ts `appIconPath()` / `trayIconPath()`
 *
...
/**
 * 5. Stage the multi-size icon pack (generated by `stage-icons.mjs`, the
 * `dist` step right before this script) at `resources/icons/` — the path
 * `electron/paths.ts` `appIconPath()`/`trayIconPath()` resolve under
 * `process.resourcesPath` in prod. Without this the packaged app has no
 * icon files at runtime: `trayIconPath()` returns `undefined` (its dev
 * fallback points into the repo checkout, absent in a packaged install)
 * and `installTray()` silently skips the tray — which on Linux is the only
 * way back from hide-on-close. Soft-skips (with a warning) when
 * `build/icons/` is empty, matching `stage-icons.mjs`'s own no-source /
 * no-ffmpeg soft-skips.
 */
function stageIcons(): void {
  const iconSrc = path.join(desktopRoot, 'build', 'icons');
  ...
  console.log(`✓ Staged icon pack -> ${dest}`);
}
...
export { stageWeb, stageBackend, stageBun, stageFfmpeg, stageIcons };
