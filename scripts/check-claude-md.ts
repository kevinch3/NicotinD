 ...
 *
 * Renames are the main source: the code moves, the index doesn't.
 *
...
const ALLOWLIST = new Map<string, string>([
  ...
  // Third-party packages: they live in package.json, not as repo symbols.
  ['castv2', 'npm package (Chromecast protocol)'],
  ['bonjour', 'npm package (mDNS discovery)'],
...
function existsInRepo(ident: string): boolean {
  try {
    const out = execFileSync(
      'git',
      ['grep', '-l', '-w', '--', ident, ':!CLAUDE.md', ':!*.lock', ':!packages/web/public'],
...
function main(): void {
  ...
  const missing = idents.filter((i) => !existsInRepo(i));
  ...
  if (!missing.length && !brokenLinks.length) {
    console.log(
      `CLAUDE.md: ${idents.length} identifiers checked, all present. No broken doc links.`,
    );
    return;
  }
